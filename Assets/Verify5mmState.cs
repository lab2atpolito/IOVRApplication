...
public class Verify5mmState : ATask
{
    ...
    public override int GetNextTaskId(int currentTask)
    {
        ...
        int nextTaskId;
        if(_needle.GetNeedleType() == NeedleType.BLUE && needle.IsOneLineVisible() && (needle.GetAnglePrecision() / 100f) >= 0.8f && (needle.GetPositionPrecision() / 100f) >= 0.8f && needle.GetCurrentState() == State.IN_BONE)
        {
